...
def generate_color_mapping(non_can_motifs_str: str, canonical_motif: str, max_mers: int) -> Dict[str, str]:
    ...
    for i, motif in enumerate(sorted(motifs_set - {canonical_motif})):
        color_mapping[motif] = DISTINCT_COLORS[i % len(DISTINCT_COLORS)]
...
def visualize_motifs(
    ...
    ax.imshow(color_stack, aspect="auto", interpolation="nearest", extent=[0, max_seq_len, 0, len(color_stack)])

    # Set x-axis and title
    ax.set_xlim(0, max_seq_len)
    ax.set_xticks([0, max_seq_len // 2, max_seq_len])
    ax.set_xticklabels(['0', str(max_seq_len // 2), str(max_seq_len)], fontsize=14)
    ax.tick_params(axis='x', which='both', bottom=True, top=False, labelbottom=True)
    ax.set_yticks([])
    ax.set_title(f'{locus} (n={len(sorted_sequences)})', fontsize=17)
    ...
    top_motifs = sorted(total_motif_counts, key=total_motif_counts.get, reverse=True)[:34]
    legend_labels = [canonical_motif] + top_motifs
    legend_colors = [colors.get(label, '#000000') for label in legend_labels]
    legend_labels = list(dict.fromkeys(legend_labels))[:35]
    ...
    fig.legend(handles=patches, loc='upper center', fontsize=14, ncol=11,
...
def main():
    ...
    parser.add_argument("--max-mers", type=int, required=True, help="Maximum number of mers")
    parser.add_argument("--vlines", type=str, default=None, help="Vertical lines (optional)")
    ...
    visualize_motifs(sequences, args.canonical_motif, args.locus, args.max_mers, vlines, 
                                     output_file_png, colors, tsv_motifs)
